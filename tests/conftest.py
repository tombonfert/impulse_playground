...
def setup_narrow_db(spark):
    ...
    container_tags = spark.createDataFrame(
        pd.read_csv(f"{base_path}/tests/data/unit_test_csv/1_container_tags.csv"),
        schema=S.CONTAINER_TAGS,
    )
    container_metrics = spark.createDataFrame(
        pd.read_csv(
            f"{base_path}/tests/data/unit_test_csv/1_container_metrics.csv",
            parse_dates=[1, 2],
        ),
        schema=S.CONTAINER_METRICS,
    )
    channel_tags = spark.createDataFrame(
        pd.read_csv(f"{base_path}/tests/data/unit_test_csv/1_channel_tags.csv"),
        schema=S.CHANNEL_TAGS,
    )
    channel_metrics = spark.createDataFrame(
        pd.read_csv(f"{base_path}/tests/data/unit_test_csv/1_channel_metrics.csv"),
        schema=S.CHANNEL_METRICS,
    )
    channels = spark.createDataFrame(
        pd.read_csv(
            f"{base_path}/tests/data/unit_test_csv/1_channels.csv",
            dtype={
...
def setup_basic_db(spark):
    ...
    container_metric_path = f"{base_path}/tests/data/basic_narrow_csv/container_metrics.csv"
    channel_metric_path = f"{base_path}/tests/data/basic_narrow_csv/channel_metrics.csv"
    channels_path = f"{base_path}/tests/data/basic_narrow_csv/channel_data.csv"
...
def narrow_db(spark, setup_narrow_db, mock_workspace_client) -> MeasurementDB:
    ...
    return MeasurementDB(cfg, mock_workspace_client)
...
def setup_key_value_store_db(spark):
    ...
    container_tags_path = f"{base_path}/tests/data/key_value_store_csv/container_metrics.csv"
